...
OPTIONS:
 ...
 -f  --file      file with csv data                    = ./csv/test.csv
...
function per(t,p)
    ...
    return t[math.max(1, math.min(#t, p))]
end
...
function Data:new(src) 
  ...
  if   type(src) == "string" 
    ...
    else for _, row in pairs(src or {}) do
            self:add(row)
        end
    end
end
...
function eg.csv(   n) 
  n=0
  csv("./csv/test.csv",function(row)
        n = n + 1;
        if n > 10 then return else oo(row) end
    end);
    return true
end
